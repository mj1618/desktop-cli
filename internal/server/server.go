...
// Server wraps the MCP server with the platform provider and cache.
type Server struct {
...
// New creates and configures an MCP server with all desktop-cli tools.
func New(cfg Config) (*Server, error) {
	...
	s := &Server{
		provider: provider,
		cache:    NewTreeCache(cfg.CacheTTL),
	}
